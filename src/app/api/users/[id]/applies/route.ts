 ...
 *      - name: type
 *        in: query
 *        description: "`'self' | 'target'` 申请类型"
 *        required: false
 *        type: string
 *      - name: status
 *        in: query
 *        description: "`'PENDING' | 'ACCEPTED' | 'REJECTED'` 申请状态"
 *        required: false
 *        type: string
 *     responses:
 *       200:
 *         description: '`ResultType<Applies[]>` 用户的申请记录'
 */
export async function GET(request: NextRequest, { params }: PathIdParams) {
  try {
    ...
    if (status && !['ACCEPTED', 'PENDING', 'REJECTED'].includes(status.toUpperCase())) {
