...
import { friendApplyService, groupApplyService } from '@/services/apply'
 ...
 *      - name: type
 *        in: query
 *        description: "`'self' | 'target' | 'group'` 申请类型，分别表示自己申请的、被申请的和群申请"
 *        required: false
 *        type: string
 *        default: self
 *        enum:
 ...
 *     responses:
 *       200:
 *         description: '`ResultType<FriendApply[] | GroupApply[]>` 用户的申请记录'
 */
export async function GET(request: NextRequest, { params }: PathIdParams) {
  try {
    ...
    if (type && !['group', 'self', 'target'].includes(type)) {
    ...
    if (!type || type === 'self') {
      const applies = await friendApplyService.getApplies(params.id, page, (status && status.toUpperCase()) as ApplyStatusType)
      return Result.success(friendApplyService.asVoList(applies))
    }
    else if (type === 'group') {
      const applies = await groupApplyService.getAppliesByUserId(params.id, page, (status && status.toUpperCase()) as ApplyStatusType)
      return Result.success(groupApplyService.asVoList(applies))
    }
    else {
      const applies = await friendApplyService.getAppliesByTargetId(params.id, page, (status && status.toUpperCase()) as ApplyStatusType)
      return Result.success(friendApplyService.asVoList(applies))
    }
  }
  catch (error: any) {
